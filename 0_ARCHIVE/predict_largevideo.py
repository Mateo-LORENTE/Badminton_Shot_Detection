...
if __name__ == '__main__':
    ...
    for video in videos : 
        ...
        out_csv_file = os.path.join(args.save_dir, f'{video}_ball.csv')
        out_video_file = os.path.join(args.save_dir, f'{video}_sortie.mp4')
        ...
        if args.eval_mode == 'nonoverlap':
            ...
            if large_video:
                dataset = Video_IterableDataset(video, seq_len=seq_len, sliding_step=seq_len, bg_mode=bg_mode, 
            ...
            if large_video:
                dataset = Video_IterableDataset(video, seq_len=seq_len, sliding_step=1, bg_mode=bg_mode,
                                                max_sample_num=args.max_sample_num, video_range=video_range)
                data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)
        ...
        if args.output_video:
            write_pred_video(video, pred_dict, save_file=out_video_file, traj_len=args.traj_len)
