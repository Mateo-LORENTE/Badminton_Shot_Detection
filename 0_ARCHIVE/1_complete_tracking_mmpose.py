...
def complete_tracking(inputs_path, videos,references,csv_court_coordinates, outputs_path):
	for videoName,reference in zip(videos,references):
		...
		for scene in similar_scenes:
			...
			for frame_number in range(start_frame, end_frame + 1,3):
				...
				for i in range(3):
					...
					# Resize image
					img = image.copy()
					image = run_mmpose(img, frame_number+i, det_model, pose_model, visualizer, df_predict_mmpose)
					
					out.write(image)


		df_predict_mmpose = df_predict_mmpose.set_index('Frame')
		...
		df_predict = pd.concat([df_predict_tracknet,df_predict_mmpose],axis=1)
